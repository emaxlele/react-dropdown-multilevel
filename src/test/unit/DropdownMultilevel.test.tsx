...
function renderDropdown(
	props: Partial<React.ComponentProps<typeof DropdownMultilevel>> = {}
) {
	return render(
		<DropdownMultilevel title="Menu Test" {...props}>
			...
			<DropdownMultilevel.Item isDisabled>
				Disabilitata
...
describe("DropdownMultilevel", () => {
	describe("Rendering", () => {
		it("renderizza il bottone con il titolo", () => {
			...
			expect(
				screen.getByRole("button", { name: "Ciao" })
			).toBeInTheDocument();
		});
	...
	describe("Navigazione da tastiera", () => {
		it("ArrowDown sul bottone apre il menu e mette a fuoco il primo item", async () => {
			renderDropdown();
			const btn = screen.getByRole("button");
			btn.focus();
		...
		it("ArrowDown nel menu sposta il focus all'item successivo", async () => {
			renderDropdown();
		...
		it("Escape nel menu chiude e riporta il focus sul bottone", async () => {
			renderDropdown();
			const btn = screen.getByRole("button");
			btn.focus();
			await userEvent.keyboard("{ArrowDown}");
