...
import { usePlaygroundStore } from "@src/test/store/playgroundStore";

/**
 * Pagina di test manuale / demo.
 * Avvia con: pnpm dev
 * Apri: http://localhost:5173
 *
 * Lo stato del log è gestito da uno store Zustand (vedi store/playgroundStore.ts)
 * per dimostrare l'integrazione con uno state manager esterno.
 */
export default function DropdownPlayground(): JSX.Element {
	...
	return (
		<div
			...
			className={
				"overflow-auto h-screen w-screen text-center items-center justify-center place-items-center"
			}
		>
			...
			<section style={{ marginBottom: 40 }}>
				...
				>
					{log.length === 0 ? (
						...
						log.map((l) => (
							<div key={l.id}>{`[${l.time}] ${l.message}`}</div>
						))
					)}
				</div>
				<button onClick={() => clearLog()} style={{ marginTop: 8 }}>
					Pulisci log
				</button>
			</section>
			...
			<section style={{ marginBottom: 40 }}>
				...
				>
					<DropdownMultilevel.Item
						onClick={() => addLog("Caso 1 → Voce 1")}
					>
						Voce 1
					</DropdownMultilevel.Item>
					<DropdownMultilevel.Item
						onClick={() => addLog("Caso 1 → Voce 2")}
					>
						Voce 2
					</DropdownMultilevel.Item>
					<DropdownMultilevel.Divider />
					<DropdownMultilevel.Item
						onClick={() => addLog("Caso 1 → Voce 3")}
					>
						Voce 3
					</DropdownMultilevel.Item>
				</DropdownMultilevel>
			...
			<section style={{ marginBottom: 40 }}>
				...
				<div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
					{(["left", "right", "top-left", "top-right"] as const).map(
						(pos) => (
							<DropdownMultilevel
								key={pos}
								title={`Position: ${pos}`}
			...
			<section style={{ marginBottom: 40 }}>
				...
				<p>
					Passa il mouse su &quot;Altre opzioni&quot; → deve aprirsi
					il sottomenu a destra
				</p>

				<DropdownMultilevel title="Menu con SubMenu">
					<DropdownMultilevel.Item
						onClick={() => addLog("Caso 6 → Voce 1")}
					>
						Voce 1
					</DropdownMultilevel.Item>

					<DropdownMultilevel.Item
						onClick={() => addLog("Caso 6 → Voce 2")}
					>
						Voce 2
					</DropdownMultilevel.Item>
					...
					>
						<DropdownMultilevel.Item
							onClick={() => addLog("Caso 6 → Sub 1")}
						>
							Sub Voce 1
						</DropdownMultilevel.Item>
						<DropdownMultilevel.Item
							onClick={() => addLog("Caso 6 → Sub 2")}
						>
							Sub Voce 2
						</DropdownMultilevel.Item>
						<DropdownMultilevel.Item
							onClick={() => addLog("Caso 6 → Sub 3")}
						>
							Sub Voce 3
						</DropdownMultilevel.Item>
					</DropdownMultilevel.SubMenu>
			...
			<section style={{ marginBottom: 40 }}>
				...
				<DropdownMultilevel title="Menu SubMenu Left" position="right">
					...
					>
						<DropdownMultilevel.Item
							onClick={() => addLog("6b → Sub 1")}
						>
							Sub Voce 1
						</DropdownMultilevel.Item>
						<DropdownMultilevel.Item
							onClick={() => addLog("6b → Sub 2")}
						>
							Sub Voce 2
						</DropdownMultilevel.Item>
					</DropdownMultilevel.SubMenu>
			...
			{/* ── Caso 8: Item con icona (lucide-react) ────────────── */}
			<section style={{ marginBottom: 40 }}>
				<h2>🖼️ Caso 8 — Item con icona (lucide-react)</h2>
